...
document.querySelector('.alt1').addEventListener('click', function () {
    productImage.style.backgroundImage = "url('img/acessories/mipowerbk.png')"
...
function adicionarCarrinho() {
    ...
    const gravarProduto = localStorage.setItem('db_miPowerbank', JSON.stringify(product))
...
function checkFavorites() {
    ...
    const productExists = inspectFavorites.some(item => item.mipowerbank);
...
favoritar.addEventListener('click', function () {
    ...
    let favProduct = {
        "mipowerbank": {
            ...
            "image": "img/acessories/mipowerbk.png",
    ...
    const productExists = getFavorite.some(item => item.mipowerbank);

    if (!productExists) {
        getFavorite.push(favProduct);
    } else {
        getFavorite = getFavorite.filter(item => !(item.mipowerbank));
