...
function adicionarCarrinho() {
    ...
    const gravarProduto = localStorage.setItem('db_premiumCharger', JSON.stringify(product))
...
function checkFavorites() {
    ...
    const productExists = inspectFavorites.some(item => item.premiumcharger);
...
favoritar.addEventListener('click', function () {
    ...
    let favProduct = {
        "premiumcharger": {
            ...
            "image": "img/acessories/scharger2.avif",
    ...
    const productExists = getFavorite.some(item => item.premiumcharger);

    if (!productExists) {
        getFavorite.push(favProduct);
    } else {
        getFavorite = getFavorite.filter(item => !(item.premiumcharger));
