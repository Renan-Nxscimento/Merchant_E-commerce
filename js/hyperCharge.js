...
function adicionarCarrinho() {
    ...
    const gravarProduto = localStorage.setItem('db_hyperCharge', JSON.stringify(product))
...
function checkFavorites() {
    ...
    const productExists = inspectFavorites.some(item => item.hypercharge);
...
favoritar.addEventListener('click', function () {
    ...
    let favProduct = {
        "hypercharge": {
            ...
            "image": "img/acessories/xypervharge120.png",
    ...
    const productExists = getFavorite.some(item => item.hypercharge);

    if (!productExists) {
        getFavorite.push(favProduct);
    } else {
        getFavorite = getFavorite.filter(item => !(item.hypercharge));
