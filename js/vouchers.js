...
resgatarCupom.forEach(cupom => {
    cupom.addEventListener('click', function () {

    if (cupom.id === 'v30Off') {
        document.getElementById('v10Off').innerHTML = 'RESGATAR CUPOM'

        const targetElementor = 'MRCT30OFF1'
    
        navigator.clipboard.writeText(targetElementor).then(() => {
            const voucherText = document.getElementById('v30Off')
        
            voucherText.innerHTML = 'Cupom resgatado!'
            voucherText.style.fontWeight = 'bold'
            voucherText.style.color = 'rgb(243, 97, 0)'
})
    } if (cupom.id === 'v10Off') {
        document.getElementById('v30Off').innerHTML = 'RESGATAR CUPOM'

        const targetElementor = 'GT10MRCT'
        
        navigator.clipboard.writeText(targetElementor).then(() => {
            const voucherText = document.getElementById('v10Off')
