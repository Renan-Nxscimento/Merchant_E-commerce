...
function adicionarCarrinho() {
    ...
    const gravarProduto = localStorage.setItem('db_samHolder', JSON.stringify(product))
...
function checkFavorites() {
    ...
    const productExists = inspectFavorites.some(item => item.samsungstand);
...
favoritar.addEventListener('click', function () {
    ...
    let favProduct = {
        "samsungstand": {
            ...
            "image": "img/acessories/sphonestand.avif",
    ...
    const productExists = getFavorite.some(item => item.samsungstand);

    if (!productExists) {
        getFavorite.push(favProduct);
    } else {
        getFavorite = getFavorite.filter(item => !(item.samsungstand));
