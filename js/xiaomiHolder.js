...
function adicionarCarrinho() {
    ...
    const gravarProduto = localStorage.setItem('db_miHolder', JSON.stringify(product))
...
function checkFavorites() {
    ...
    const productExists = inspectFavorites.some(item => item.xcarstand);
...
favoritar.addEventListener('click', function () {
    ...
    let favProduct = {
        "xcarstand": {
            ...
            "image": "img/acessories/xcarstand.png",
    ...
    const productExists = getFavorite.some(item => item.xcarstand);

    if (!productExists) {
        getFavorite.push(favProduct);
    } else {
        getFavorite = getFavorite.filter(item => !(item.xcarstand));
