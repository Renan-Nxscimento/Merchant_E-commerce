...
document.querySelector('.alt2').addEventListener('click', function () {
    productImage.style.backgroundImage = "url('img/watches/AltImgs/vwatchbp/alt1.webp')"
    backgroundImage = productImage.style.getPropertyValue('background-image').slice(4, -1).replace(/["']/g, "")
})

document.querySelector('.alt3').addEventListener('click', function () {
    productImage.style.backgroundImage = "url('img/watches/AltImgs/vwatchbp/alt2.webp')"
    backgroundImage = productImage.style.getPropertyValue('background-image').slice(4, -1).replace(/["']/g, "")
})

document.querySelector('.alt4').addEventListener('click', function () {
    productImage.style.backgroundImage = "url('img/watches/AltImgs/vwatchbp/alt3.webp')"
    backgroundImage = productImage.style.getPropertyValue('background-image').slice(4, -1).replace(/["']/g, "")
})

document.querySelector('.alt5').addEventListener('click', function () {
    productImage.style.backgroundImage = "url('img/watches/AltImgs/vwatchbp/alt4.webp')"
    backgroundImage = productImage.style.getPropertyValue('background-image').slice(4, -1).replace(/["']/g, "")
})
...
function adicionarCarrinho() {
    ...
    const gravarProduto = localStorage.setItem('db_viWatchBp', JSON.stringify(product))
...
function checkFavorites() {
    ...
    const productExists = inspectFavorites.some(item => item.vivowatchbp);
...
favoritar.addEventListener('click', function () {
    ...
    let favProduct = {
        "vivowatchbp": {
            ...
            "image": "img/watches/avivowatchBP.png",
    ...
    const productExists = getFavorite.some(item => item.vivowatchbp);

    if (!productExists) {
        getFavorite.push(favProduct);
    } else {
        getFavorite = getFavorite.filter(item => !(item.vivowatchbp));
